...
export default async function Home({ params }) {
  ...
  const { object } = await generateObject({
    ...
    prompt: `Create a long-form Wikipedia style article about: ${input}. Include an info card with relevant categories and their corresponding values. For each section, provide a header and detailed content, and create many different sections.`,
  });
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white shadow-md relative">
      ...
      <p className=" mb-6">{object.wikipedia_article_initial_summary}</p>
      <div className="space-y-4">
        {object.wikipedia_article_sections.map((section, index) => (
          <div key={index}>
